...
int helper(int n,vector<int> &coins){
  ...
  int take = coins[n] + helper(n-1,coins);
  int nottake = helper(n-1,coins);
  if(take!=0 && ans.find(take)!=ans.end())ans.insert(take);
  if(nottake!=0 && ans.find(nottake)!=ans.end())ans.insert(nottake);
...
void MahavirCoder(){
  int n;
	int x;
	cin>>n>>x;
  vc price(n),pages(n);cin>>price>>pages;
