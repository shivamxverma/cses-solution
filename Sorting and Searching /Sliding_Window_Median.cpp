...
#define lob(a, x) (lo_bound((a).begin(), (a).end(), (x)) - (a).begin())
#define upb(a, x) (up_bound((a).begin(), (a).end(), (x)) - (a).begin())
...
void er(int pr){
	if(up.find(pr)!=up.end()){
			up.erase(up.find(pr));
		}
		else if(lo.find(pr)!=lo.end()) {
      lo.erase(lo.find(pr));
		}
    if (lo.empty()) {
		lo.insert(*up.begin());
...
void MahavirCoder() {
  ...
  int cm = *lo.rbegin();
	int answer = (((k+1)/2)*cm - x) + (y - (k/2)*cm);
	for(int i=k ; i<n ; i++){
		int pr = v[i-k];
		int xx = v[i];
		if (k == 1) {
