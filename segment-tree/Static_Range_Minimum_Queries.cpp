...
const int N = 200007;
vector<int> segTree(4*N+7, INT_MAX);
...
void build(int s, int e, int i, vector<int> &a) {
    ...
    build(s, m, left, a);
    build(m+1, e, right, a);
    segTree[i] = min(segTree[left], segTree[right]);
}

int query(int s, int e, int l, int r, int i) {
    if(l > e || r < s) return INT_MAX;
    if(l <= s && e <= r) return segTree[i];
    int m = s + (e-s)/2; // Changed ll to int
    return min(query(s, m, l, r, 2*i), query(m+1, e, l, r, 2*i+1));
}
