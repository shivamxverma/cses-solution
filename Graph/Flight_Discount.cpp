...
void dfs(int start,vector<vector<int>> &adj,vector<bool> &mark,vector<int> &parent){
  mark[start] = true;
  for(auto edge : adj[start]){
    if(!mark[edge]){
      parent[edge] = start;
      dfs(edge,adj,mark,parent);
    }
  }
}
...
void Dijkstra(int s, int n, vector<ll> &dist, vector<int> &parent, vector<pair<int, ll>> *adj) {
	...
	while (!q.empty()) {
		...
		if (d_v != dist[v])
			continue;
		for (auto edge : adj[v]) {
			if (dist[v] + edge.s < dist[e]) {
				dist[e] = dist[v] + edge.s;
				parent[e] = v;
				q.push({dist[e], e});
			}
		}
...
// Topological sort
void dfs(int start, vector<int>*edges, bool *markited, vector<int>& ans)
{
	markited[start] = true;
...
{
	vector<int> v1;
	bool *markited = new bool[n]();
	for (int i = 0; i < n; i++)
	{
		if (markited[i])
			continue;
		dfs(i, edges, markited, v1);
	}
...
vector<bool> mark;
vector<int> dist;
vector<pair<int,set<int>>> Allpath;
 
vector<vector<pair<int,int>>> adj(int n,int m){
...
void dfs(int start, int rsum , set<int> path) {
	if(path.find(start+1)==path.end())path.insert(start + 1);
	else return ;
	if (start == n - 1) {
		Allpath.pb({rsum, path});
		return;
	}
	for (auto [e, l] : graph[start]) {
		if (!mark[e]) {
			mark[e] = true;
			dfs(e, rsum + l, path);
			mark[e] = false;
